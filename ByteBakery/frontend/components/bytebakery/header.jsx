...
export function Header() {
  ...
  const { isAuthenticated, usuario, login, register, logout } = useAuth();
  ...
  /* Estado para controlar el menu hamburguesa en movil (fix hidratacion Next.js) */
  ...
  /* Escuchar evento personalizado para abrir el modal de login desde otros componentes */
  useEffect(() => {
  ...
  const handleLogin = async (e) => {
    ...
    if (result.ok) {
      setToastMsg("¡Bienvenido, " + result.usuario.nombre + "!");
      setShowToast(true);
      setShowAuth(false);
  ...
  return (
    <>
      ...
      <Navbar bg="white" expand="lg" fixed="top" className="border-bottom" style={{ boxShadow: "0 1px 6px rgba(0,0,0,0.06)" }}
        expanded={navExpanded} onToggle={(val) => setNavExpanded(val)}>
        <Container>
          {/* Marca de la empresa con logo */}
          <Navbar.Brand onClick={() => navigate('/')} className="font-serif fw-bold d-flex align-items-center gap-2" style={{ color: "var(--primary)", cursor: "pointer" }}>
            <img src="/ByeBakery.png" alt="ByteBakery Logo" width="32" height="32" />
            ByteBakery
          </Navbar.Brand>

          <Navbar.Toggle id="main-navbar" aria-controls="main-navbar" />
          <Navbar.Collapse id="main-navbar">
            {/* Enlaces centrales del menu */}
            <Nav className="mx-auto gap-1">
              <Nav.Link onClick={() => { navigate('/'); setNavExpanded(false); }} style={{ cursor: "pointer" }}>Inicio</Nav.Link>
              <NavDropdown title="Productos" id="prod-dd">
                <NavDropdown.Item onClick={() => { navigate('/catalogo'); setNavExpanded(false); }}>Catalogo</NavDropdown.Item>
                {isAuthenticated && usuario?.rol !== "admin" && (
                  <>
                    <NavDropdown.Divider />
                    <NavDropdown.Item onClick={() => { navigate('/pedidos'); setNavExpanded(false); }}>Realizar Pedido</NavDropdown.Item>
                    <NavDropdown.Item onClick={() => { navigate('/promociones'); setNavExpanded(false); }}>Promociones Especiales</NavDropdown.Item>
                  </>
                )}
              </NavDropdown>
              <Nav.Link onClick={() => { navigate('/nosotros'); setNavExpanded(false); }} style={{ cursor: "pointer" }}>Nosotros</Nav.Link>
              <Nav.Link onClick={() => { navigate('/contacto'); setNavExpanded(false); }} style={{ cursor: "pointer" }}>Contacto</Nav.Link>
            </Nav>

            {/* Area de usuario y carrito a la derecha */}
            <Nav className="align-items-center gap-2">
              {/* Boton del carrito de compras (oculto para admin) */}
              {usuario?.rol !== "admin" && (
              ...
              {isAuthenticated ? (
                <>
                  <Nav.Link onClick={() => navigate('/dashboard')} className="fw-bold" style={{ color: "var(--primary)", cursor: "pointer" }}>
                    Dashboard
                  </Nav.Link>
                ...
                <Tab eventKey="login" title="Iniciar Sesion">
                  <Form onSubmit={handleLogin}>
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control type="email" placeholder="correo@ejemplo.com" value={correo}
                        onChange={e => setCorreo(e.target.value)} required size="sm" />
                    </Form.Group>
                    <Form.Group className="mb-3">
                  ...
                  {regSuccess ? (
                    ...
                    <Form onSubmit={handleRegister}>
                      ...
                      <Form.Group className="mb-1">
                        <Form.Label className="small fw-bold">Contrasena</Form.Label>
                        <div className="input-group input-group-sm">
                          <Form.Control type={showRegPw ? "text" : "password"} placeholder="Tu contrasena"
        ...
        <Modal.Body>
          {items.length === 0 ? (
            <div className="text-center py-5">
              ...
              <Button variant="link" onClick={() => { setShowCart(false); navigate('/catalogo'); }}
              ...
              <div className="pt-4">
                ...
                {usuario?.rol === "admin" ? (
                  ...
                  <Button
                    ...
                    style={{ backgroundColor: "#1a1a1a", color: "#fff" }}
                    onClick={() => { setShowCart(false); navigate("/pedidos"); }}
                  >
                    Confirmar Pedido
                  </Button>
                )}
